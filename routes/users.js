...
router.get("/signup", (req, res) => {
  if (req.session.errmail) {
    res.render("./user/signup-page", {
      admin: false,
      mail: "This Mail Address Already Existing",
    });
...
router.post("/signup", (req, res) => {
  userbase
    .Checking_Mailaderss_Is_Existing(req.body.email)
    .then(async (resl) => {
      if (resl.mail) {
...
router.get("/otp", (req, res) => {
  if (req.session.otp) {
    res.render("./user/otp-validation", {
      admin: false,
      otperr: "Invalid OTP Please Enter Correct OTP...",
    });
    ...
    userbase.Get_Cart_Count(req.session.user._id).then((count) => {
      if (Globalqut) {
        res.render("./user/single-product", {
          ...
          product,
          count,
          cuterr: "Enter The Quantity inside",
        });
        Globalqut = false;
      } else {
        if (successcut) {
          res.render("./user/single-product", {
            ...
            product,
            count,
            succ: "Your Product Successfully Carted",
          });
          successcut = false;
        } else {
          res.render("./user/single-product", {
...
router.post("/cart", verfyUserLogin, (req, res) => {
  userbase.Get_Single_Products(req.query.id).then((pro) => {
    if (req.body.cut > pro.qun || req.body.cut <= 0) {
      ...
      userbase
        .Do_Cart(req.session.user._id, req.query.id, req.body.cut)
        .then((data) => {
          console.log(data);
        });
      successcut = true;
  ...
  userbase.Get_products_From_Cart_Base(req.session.user._id).then((pro) => {
    ...
    userbase.Get_Total_price(req.session.user._id).then((total) => {
      ...
      userbase.Get_order_placement(req.body).then(async (orderId) => {
        await qrcode.GenerateOrder_Qr_Code(orderId).then((data) => {
          console.log(data);
...
router.get("/notification", verfyUserLogin, (req, res) => {
  userbase
    .Get_user_message_From_admin(req.session.user._id)
    .then(async (msg) => {
      //console.log(msg);
      await userbase
        .Get_product_Details_For_Chat(req.session.user._id)
        .then((pro) => {
          res.render("./user/notification-page", {
            admin: false,
            user: req.session.user,
            pro,
          });
